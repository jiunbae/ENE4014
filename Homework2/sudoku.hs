...
varlist = [fromInteger x|x<-varList]

parseMatrix :: [Int] -> [[Int]]
parseMatrix list = [drop (9*i) (take (9*(i+1)) list) | i <- [0..8]]

parseSection :: [Int] -> Map Int [(Int, Int)]
parseSection list = Map.fromList[(x, [(fromIntegral (fst y `div` 9), fromIntegral (fst y `mod` 9)) | y <- zip [0..] blkList, snd y == x]) | x <- [1..9]]

nextStep :: [[Int]] -> (Int, Int) -> (Int, Int)
nextStep matrix coord = if null valid
                            then (0, -1)
                            else head valid
                                where valid = [(x, y) | x <- drop (fst coord) ([0..8] ++ [0..8]), y <- drop (snd coord) ([0..8] ++ [0..8]), matrix!!x!!y == 0]
...
isValid matrix sections coord e = if all (==True) ([e /= matrix!!(fst coord)!!x | x <- [0..8]] ++ [e /= matrix!!y!!(snd coord) | y <- [0..8]])
